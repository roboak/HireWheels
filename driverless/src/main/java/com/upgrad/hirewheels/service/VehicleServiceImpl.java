...
public class VehicleServiceImpl implements VehicleService {
    ...
    public List<VehicleDetailResponse> getAvailableVehicles(String categoryName, Date pickUpDate,Date dropDate) {
        List<Vehicle> returnedVehicleList = new ArrayList<>();
        vehicleCategoryRepo.findByVehicleCategoryName(categoryName).getVehicleSubCategories().forEach(a-> a.getVehicle().forEach(b->returnedVehicleList.add(b)));
        List<Integer> bookedVehicles = new ArrayList<>();
        bookingRepository.findByPickUpDateGreaterThanEqualAndDropOffDateLessThanEqual(pickUpDate, dropDate).forEach(a->{ bookedVehicles.add(a.getVehicleWithBooking().getVehicleId());});
        ...
        for (Vehicle v : returnedVehicleList) {
            if (approvedVehicles.contains(v.getVehicleId())) {
                if(!bookedVehicles.contains(v.getVehicleId())){
                    ...
                    y.setVehicleOwner(v.getUsers().getUserId());
